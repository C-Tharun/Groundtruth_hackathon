...
def main():
    ...
    with st.sidebar:
        ...
        use_llm = st.radio(
            "Summary generation:",
            ["Quick (Template)", "AI (LLM)"],
            index=0,
            help="Quick mode uses template summary. AI mode requires API key."
        )
        use_llm = use_llm == "AI (LLM)"
        
        if use_llm:
            api_key_set = bool(os.getenv('GROQ_API_KEY') or os.getenv('OPENAI_API_KEY'))
            if not api_key_set:
                st.warning("⚠️ No API key found. Set GROQ_API_KEY or OPENAI_API_KEY environment variable.")
    ...
    with col_gen1:
        st.info("Click the button below to generate a PowerPoint presentation with all insights.")
    ...
    if generate_button:
        with st.spinner("Generating report... This may take a moment."):
            try:
                ...
                from src.utils import llm_generate_summary
                ...
                st.info("💡 To convert to PDF, use LibreOffice: `libreoffice --headless --convert-to pdf outputs/automated_report.pptx --outdir outputs/`")
